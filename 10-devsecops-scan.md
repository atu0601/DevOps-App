...
# Scanner une image
trivy image devops-app-app

# Scanner uniquement les vulnérabilités critiques et hautes
trivy image --severity HIGH,CRITICAL devops-app-app

# Scanner et échouer si vulnérabilités critiques
trivy image --severity CRITICAL --exit-code 1 devops-app-app

# Format JSON pour CI
trivy image --format json --output results.json devops-app-app
